...
* 再进行调用 HashMap 的 hash() 方法进行二次哈希
  * 二次 hash() 是为了让hashcode的高位数据也参与运算，获得一个新的hash码，减小hash碰撞的概率，让哈希分布更为均匀【参考：https://blog.csdn.net/qq_32099833/article/details/124928031】
...
* 线程并发时若多个线程put节点时桶下标一致，可能会因为执行时机的问题导致后执行的线程覆盖前一个线程put的节点
...
* 枚举类在编译时其实也是会编译为final修饰的class类型，枚举类值其实就对应了class的常量，常量值为当前枚举类型对象，在静态代码块中创建
